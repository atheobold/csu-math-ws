...
(like we did above):  
    ...
    run the code `Cat + 2`, you will get an error saying that `Cat` does not
    exist  
...
The value of `years_post_birthday` is still 16 because you have not
re-run the line `years_post_birthday <- year - 1989` since changing the value of `year`.
...
`num_logical` gets converted into a `1` before it gets converted into `"1"` in
`combined_logical`.
...
__Can you draw a diagram that represents the hierarchy of how these data types
are coerced?__
